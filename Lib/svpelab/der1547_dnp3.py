...
def der1547_info():
    """
    Returns the DNP3 information dictionary.

    This function provides access to the DNP3 information dictionary, which contains
    details about the DNP3 protocol configuration and parameters.
    """
    return dnp3_info
...
def params(info, group_name):
    """
    Adds DNP3 specific parameters to the info object for DER configuration.
    
    Parameters:
    -----------
        info: The parameter information object to add parameters to
        group_name: The name of the parameter group
        
    The function configures:
    - DNP3 communication parameters (IP addresses, ports)
    - Outstation simulation settings
    - DER simulator paths and configuration
    - DBus settings
    """
...
class DER1547(der1547.DER1547):
    """
    DNP3-specific implementation of IEEE 1547 DER interface.
    
    This class extends the base DER1547 class to provide DNP3 protocol-specific 
    functionality for DER communication and control. It implements all required 
    IEEE 1547 functions using DNP3 point mappings and data structures.
    
    Key features:
    - Translates between abstract 1547 parameters and DNP3 point addresses
    - Handles DNP3 agent communication and outstation management
    - Provides automatic scaling between DNP3 raw values and engineering units
    - Supports both simulated and physical DNP3 outstations
    - Implements curve management for volt-var, freq-watt and other functions
    
    The class uses standardized DNP3 point mappings from the DNP3 Application Note 
    for DER function blocks (AN2018-001 DRAFT).
    """
    ...
    def scan(self, scan_type):
        """
        Scans the outstation for data and returns the results.
        
        Parameters:
        -----------
            scan_type (str): The type of scan to perform on the outstation.

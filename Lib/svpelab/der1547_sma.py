...
class DER1547(der1547.DER1547):
    """
    SMA-specific implementation of the IEEE 1547 DER interface.

    This class extends the base DER1547 class and provides concrete implementations for SMA inverters by:
    1. Mapping IEEE 1547 parameter names to SMA-specific parameter names
    2. Converting between IEEE 1547 units and SMA units (e.g., kW to W)
    3. Implementing device-specific functions like:
    - Nameplate data retrieval with SMA register mappings
    - Power factor control with SMA sign conventions
    - Volt-var control with SMA curve formats
    - Frequency-watt control with SMA droop settings
    - Voltage and frequency ride-through settings
    - Power limiting functionality

    Key differences from parent class:
    - Uses der_sma.py for actual device communication
    - Handles SMA-specific parameter mappings and unit conversions
    - Implements SMA curve formats for various control modes
    - Manages SMA-specific state monitoring and alarms


    methods:
    --------
    param_value(name): Get parameter value by name
    config(): Get device configuration
    open(): Open communication with device
    close(): Close communication with device
    info(): Get device information
    get_nameplate(): Get nameplate information (IEEE 1547 Table 28)
    get_settings(): Get device settings
    set_settings(params): Set device settings
    get_configuration(): Get device configuration parameters
    get_monitoring(): Get present operating conditions
    get_const_pf(): Get constant power factor mode settings
    set_const_pf(params): Set constant power factor mode settings
    get_pf(): Get frequency-active power mode settings
    set_pf(params): Set frequency-active power mode settings
    get_p_lim(): Get maximum active power limit settings
    set_p_lim(params): Set maximum active power limit settings
    get_qp(): Get watt-var mode settings
    set_qp(params): Set watt-var mode settings
    get_pv(): Get volt-watt mode settings
    set_pv(params): Set volt-watt mode settings
    get_ui(): Get unintentional islanding settings
    set_ui(params): Set unintentional islanding settings
    get_ov(): Get overvoltage trip settings
    set_ov(params): Set overvoltage trip settings
    get_uv(): Get undervoltage trip settings
    set_uv(params): Set undervoltage trip settings
    get_of(): Get overfrequency trip settings
    set_of(params): Set overfrequency trip settings
    get_uf(): Get underfrequency trip settings
    set_uf(params): Set underfrequency trip settings
    set_cease_to_energize(params): Set cease to energize and trip settings

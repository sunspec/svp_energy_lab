...
class NET(object):
    """
    Network Capture System class for handling network-related operations.

    Methods:
    - info()                            : Return information string for the NET device
    - get_packets(iface=None, timeout=None, bpf_filter=None, filename=None, count=None): Start data capture and collect packets
    - print_capture(n_packets=None)     : Print captured packets
    - get_signal_data()                 : Convert packets into dict of information for the given application
    - save_packets()                    : Save pcap to net_dir directory
    - filter_packets()                  : Apply filter on the captured packets
    """
...
def net_scan():
    """
    Scans for NET modules in the current directory and imports them.
    
    The `net_scan()` function scans all files in the current directory that match the pattern `net_*.py`, imports the corresponding 
    modules, and adds them to the `NET_modules` dictionary, keyed by the 'mode' value from the `net_info()` function of the module.
    
    If a module does not have a `net_info()` function, or if an exception occurs during the import, the module is removed from 
    `sys.modules` to prevent further attempts to import it.
    """
    global NET_modules
